...
function preload() {
  // CSV is in public/, sketch is in public/p5/
...
function draw() {
  ...
  yearProgress = yearIndex / (phData.length - 1);
  ...
  if (d.year === 1985) {
    ...
    // horizontal-only drift, farther away from 1985
    let driftX = acidity * 120 + idxFromBase * 6;
    let driftY = 0;

    let wobbleStrength = lerp(1, 2, acidity);
    let spiralExp = lerp(1.9, 2.0, acidity);
    let warpAmount = lerp(0.0, 0.03, acidity);
  ...
  // Static 1985 label near yellow shell
  textAlign(LEFT, CENTER);
  let label1985X = width * 0.2;
  let label1985Y = baseY - 200;
  text("1985", label1985X, label1985Y);

  // Moving label for current pink shell year
...
function drawShell(
  ...
  for (let b = 0; b < bands; b++) {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      let extraWobble =
        map(noise(bt * 4.0, t * 7.0 + phase), 0, 1, -1, 1) * wobbleAmp;
